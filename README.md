...
# 프로젝트 선정 이유
#### 
대학 전공 선택은 개인의 진로와 직결되는 중요한 결정이지만,
많은 학생들이 자신의 성향을 체계적으로 분석하지 못한 채 전공을 선택하는 경우가 많다.
본 프로젝트는 이러한 문제를 해결하고자
사용자의 성향을 수치화하고 데이터 기반으로 분석하여 전공을 추천하는 프로그램을 목표로 선정되었다.
특히 다음과 같은 점에서 의미를 가진다.
- 주관적 판단이 아닌 객관적인 수치 기반 추천
- 만족도 피드백을 통한 반복적 추천 구조
- 주전공 확정 후 복수전공까지 고려한 확장성

# 코드 설명

### 1. 전공 데이터 구성
총 30개의 대학 전공 데이터를 사용
각 전공은 다음 9가지 기준으로 수치화됨 (1~10)
| 기준     | 설명                   |
| ------- | ----------------------|
| 팀플선호   | 팀 프로젝트 선호도        |
| 프로젝트선호 | 프로젝트 중심 학습 선호    |
| 학습량    | 학습 및 과제량 수용도      |
| 취업선호   | 취업 중심 성향           |
| 발표선호   | 발표 및 커뮤니케이션 선호   |
| 창의성    | 창의적 활동 선호          |
...
| 진로다양성  | 진로 선택 폭            |
| 사고력    | 문제 해결 및 사고력        |
전공별 특성을 반영하기 위해 공학, 상경, 사회과학 등의 분류 태그를 추가하였다.

### 2. 사용자 성향 입력
사용자는 각 기준에 대해 1~10 점으로 자신의 성향을 입력
입력 오류를 방지하기 위해 예외 처리를 포함한 입력 함수 구현
